...
$select=$_POST['select'];
  switch ($select) {
    ...
    $sql = "SELECT * FROM `book` ORDER BY price";
      ...
      case "ratingMax":
      $sql = "SELECT * FROM `book` ORDER BY showrating DESC ";
      break;  
  } 
} 
?>
        ...
        ?>
          <div class="col-md-3 mb-2 shadow-lg" book-price = "<?= $row['Price'];?>">
            <div class="card-deck">
              <div class="card border-secondary">
                ...
                <div class="card-img-overlay">
                  <h6 style="margin-top: 175px;" class="text-light bg-info text-center rounded p-1">
                    <form style="width:auto;" action="perexodnik.php" method="post">
                    <span><input style="display:none;" type="text" name="isbn" value="<?= $row['ISBN'] ?>">

                    <input style="background: none; border: none;" type="submit" value="<?= $row['Title']; ?>"></span>
                    </form>
                  </h6>
                </div>
                <div class="card-body">
             ...
             <button class="add-to-cart-recs btn btn-success btn-block" data-id="<?= $row['ISBN']?>">Add to Cart</button>
          </div>
...
<script>
  var cart = {};
   $('.add-to-cart-recs').on('click', addToCart);
